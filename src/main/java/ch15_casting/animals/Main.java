...
public class Main {
    public static void main(String[] args) {
        ...
        // 업캐스팅
        Animal animal1 = new Dog();     // 방법 # 1 A a = new B();    동물이 소리를 냅니다.
...
//        animal2.fetch();  // 불가능
        ...
        System.out.println("animal2는 animal이랑 사실상 같다고 아까 수업해서 Animal 인스턴스 하나 생성");
        ...
        dog2.fetch();
        animal2.makeSound();
        ((Dog) animal2).fetch();        // 47번 라인이 있기 때문에 자동완성으로 바로 다운캐스팅이 이루어집니다.

//        Dog dog3 = (Dog) animal3;
//        dog3.makeSound();
        /*
            53번 라인과 54번 라인은 남겨두었을 때 오류가 발생하지 않는데
            ...
            그래서 지금 경우의 수가 animal**이라는 객체가 있을 때 개가 Animal() 생성자로 생성됐는지
        ...
        if (animal4 instanceof Dog) {   // 얘가 true라면 animal4 객체는 Aniaml animal4 = new Dog();로 생성됨
            Dog dog4 = (Dog) animal4;   // 그래서 다운캐스팅이 가능하다.

            dog4.makeSound();
            dog4.fetch();       // 특히 애를 실행시키더라도 오류가 발생하지 않는다.
        }
