...
- Java 8에서 도입된 '함수형 프로그래밍(Functional Programming)'을 지원하기 위한 
표현식에 해당합니다.
...
// 메서드를 호출 
    ...
    // 위의 식이 기존의 방식
    Runnable lambadRunable = () -> System.out.println("Hello, Java Lambda!");
...
- 그래서 '사용자 정의 함수형 인터페이스'를 작성할 때 클래스 래벨에 '@FunctionalInterface'
를 명시해줍니다.
```java
@FunctionalInterface
...
- Supplier<T> - T get();           ca112()
- Consumer<T> - void accept(T t);  ca113()
- Function<T, R> - R apply(T t);   call4()
- Predicate<T> - boolean test(T t);   -> 얘는 return 타입이 고정이라 call1~call4로 묶이 애매하지만 굳이 따지면 call4()

1. Runnable
- 매개변수도 없고 반환값도 없는 단순 실행을 위한 인터펭이스
...
6. 함수형 인터페이스 조합
- Java 8에서의 함수형 인터페이스끼리 조합도 가능합니다.
...
public static void main(String[] args) {
    ...
    System.out.println(startsWithAndEndsWithX.test("Alex"));        // true
}
...
1. filter() -> Collection 자료형의 특정 element들만 추출할 때 사용하는 메서드, 람다식 사용 
2. map() - Java Collection 자료형과는 다른 형태입니다. 오히려 python에서의 map함수와 유사한 형태로
    Collection의 내부 element들에게 각각 함수/메서드를 적용하는 형태 
    즉, .map(String::toUpperCase)의 해석은 englishNames 리스트에서 a로 시작한 것만 걸러내는 결과값을(.filter()이후에 작성됐으니까)
    인리스트의 각 element들(`["alice", "anya"]`)에 toUpperCase()를 적용했다고 볼 수 있습니다. 그러면 결과값은 
    `["ALICE", "ANYA"]`가 됩니다.
